...
if(lambCount > chickenCount && lambCount > beefCount){
    console.log("lambCount wins!")
    Answer("salmon")
}



function Answer(searchValue){
    console.log("Start Function Answer()")
    var settings = {
               "async": true,
               "crossDomain": true,
            ...
            foodRandom = Math.floor(Math.random() * response.results.length)
            console.log(response.results[foodRandom])
            });
}
...
$.ajax({
  ...
  foodRandom = Math.floor(Math.random() * response.drinks.length)
            console.log(response.drinks[foodRandom])
}); 
